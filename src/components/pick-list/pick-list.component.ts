...
export class PickListComponent implements OnInit {
  ...
  uniqueSizesForPick = computed(() => {
    const s = [...new Set(this.activePickLines().map(l => l.size))];
    return s.sort((a, b) => {
  ...
  // For item-wise: group candidate lines by SKU for selection table
  ...
  getPickStatusForOrder(orderId: string): 'not_started' | 'partial' | 'completed' {
    const pls = this.pickLists().filter(pl => (pl.salesOrderIds ?? [(pl as any).salesOrderId ?? '']).includes(orderId));
  ...
  getStockForLine(styleNo: string, color: string, size: string, sleeveType?: string): number {
    return Number(this.inventory().find(i =>
      i.styleNo === styleNo && i.color === color && i.size === size &&
  ...
  getAlreadyPickedQty(orderId: string, styleNo: string, color: string, size: string, sleeveType?: string): number {
    return this.pickLists()
        .filter(pl => (pl.salesOrderIds ?? [(pl as any).salesOrderId ?? '']).includes(orderId))
        .flatMap(pl => pl.items)
        .filter(l => l.salesOrderId === orderId && l.styleNo === styleNo &&
        l.color === color && l.size === size && (l.sleeveType ?? '') === (sleeveType ?? ''))
  ...
  orderPickBadge(id: string): string {
    ...
    return s === 'completed' ? 'bg-green-100 text-green-800' : s === 'partial' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-600';
  }
  ...
  plTypeBadge(t: PickListType): string {
    return { direct:'bg-indigo-100 text-indigo-800', combined:'bg-purple-100 text-purple-800', itemwise:'bg-teal-100 text-teal-800' }[t] ?? '';
  }

  plTypeLabel(t: PickListType): string {
    return { direct:'Direct', combined:'Combined', itemwise:'Item-wise' }[t] ?? '';
  }
  ...
  getSizeTotal(size: string): number {
    return this.activePickLines().filter(l => l.size === size).reduce((s, l) => s + (l.pickQty || 0), 0);
  }
  ...
  proceedFromOrderSelection() {
    ...
    if (type === 'itemwise') {
      this.mode.set('select-items');
    } else {
      this.mode.set('pick');
    }
  }

  // ── Item selection (item-wise only) ───────────────────────────────────────────
  toggleItemSelection(key: string) {
    this.activePickLines.update(lines =>
      lines.map(l => {
        const lKey = `${l.styleNo}||${l.color}||${l.size}||${l.sleeveType ?? ''}`;
  ...
  selectAllItems() {
    this.activePickLines.update(l => l.map(x => ({ ...x, selected: true })));
  }

  clearItemSelection() {
    this.activePickLines.update(l => l.map(x => ({ ...x, selected: false })));
  }
  ...
  setPickQty(line: PickListLineItem, value: string | number) {
    ...
    this.activePickLines.update(ls => ls.map(l =>
      l.salesOrderId === line.salesOrderId && l.styleNo === line.styleNo &&
      l.color === line.color && l.size === line.size && (l.sleeveType ?? '') === (line.sleeveType ?? '')
        ? { ...l, pickQty: qty } : l
    ));
  }
  ...
  async confirmPick() {
    ...
    if (!pickingLines.length) {
      Swal.fire({ icon: 'warning', title: 'Nothing to Pick', text: 'Set pick quantity for at least one item.' });
      return;
    }
    ...
    try {
      ...
      for (const order of orders) {
        ...
        if (oLines.every(l => l.balanceQty === 0)) {
      ...
      Swal.fire({ icon: 'error', title: 'Error', text: err?.message ?? 'Failed to confirm.' });
    } finally {
  ...
  private buildPrintHtml(
    ...
    sizes: string[], groups: { styleNo: string; color: string; sleeve: string;
      lines: { size: string; orderedQty: number; pickedQty: number; balanceQty: number }[] }[]
  ): string {
    const sb = status === 'Completed' ? '#16a34a' : '#d97706';
    const sbg = status === 'Completed' ? '#d1fae5' : '#fef3c7';
    ...
    <div style="display:flex;justify-content:space-between;border-bottom:2px solid #1e293b;padding-bottom:8px">
      <div><h2>${title}</h2>
        <p style="margin:2px 0;color:#64748b;font-size:11px">${subTitle}</p>
    ...
    <table>
      ...
      <tfoot><tr>
        ...
        ${sizes.map(sz=>{
          const sp=groups.flatMap(g=>g.lines).filter(l=>l.size===sz).reduce((s,l)=>s+(l.pickedQty||0),0);
  ...
  printActivePickList() {
    ...
    const groups = [...map.entries()].map(([key, lines]) => {
      ...
      return {
        ...
        lines: lines.map(l => ({ size: l.size, orderedQty: l.orderedQty, pickedQty: l.pickQty, balanceQty: Math.max(0, l.balanceQty - l.pickQty) }))
      };
    });
    ...
    const html  = this.buildPrintHtml(
      ...
      this.totalOrderedQty(), this.totalPickQty(), this.totalBalanceQty(),
      sizes, groups
    );
  ...
  printSavedPickList(pl: PickList) {
    ...
    const sizeSet = new Set(items.map(i => i.size));
    ...
    const groups = [...gMap.entries()].map(([key, lines]) => {
      const [styleNo, color, sleeve] = key.split('||');
      return { styleNo, color, sleeve: sleeve || '',
        lines: lines.map(l => ({ size: l.size, orderedQty: l.orderedQty, pickedQty: l.pickedQty, balanceQty: l.balanceQty })) };
    });
    ...
    const html = this.buildPrintHtml(
      pl.pickListNo, `${pl.salesNos.join(', ')} · ${pl.clientName}`,
